...
myLayoutHook = avoidStruts $ onWorkspaces ["0_9", "1_9"] layoutGrid $ layoutTall ||| layoutGrid ||| layoutTabbed
  where
    layoutTall = mkToggle (NBFULL ?? EOT) . named "tall" $ draggingVisualizer $ smartBorders $ mouseResizableTile { masterFrac = 0.65, draggerType = FixedDragger 0 30}
    layoutGrid = mkToggle (NBFULL ?? EOT) . named "grid" $ draggingVisualizer $ smartBorders $ Grid False
    layoutTabbed = mkToggle (NBFULL ?? EOT) . named "full" $ smartBorders $ tabbed shrinkText myTabTheme
    myTabTheme = def
      { fontName            = "xft:Roboto:size=8:bold"
      , activeColor         = grey1
      , inactiveColor       = grey1
...
myStatusBarSpawner (S s) = do
                    pure $ statusBarPropTo ("_XMONAD_LOG_" ++ show s)
                          ("xmobar -x 1" ++ show s ++ " ~/.config/xmonad/xmobar/xmobar" ++ show s ++ ".config")
                          (pure $ myXmobarPP (S s))
